...
module.exports = class QuickBotInvite extends Plugin {
  async startPlugin() {
    ...
    const GuildChannelUserContextMenu = await getModule(
      (m) =>
        m.default && m.default.displayName === "GuildChannelUserContextMenu"
    );
    ...
    inject(
      ...
      (args, res) => {
        let guilds = [];
        const clonableGuilds = Object.values(
          this.getFlattenedGuilds()
        ).filter((guild) =>
          this.hasPermission(guild.id, Permissions.MANAGE_GUILD)
        );
            ...
            React.createElement(menu.MenuItem, {
              ...
              action: async () => {
                const win = await popout(
                  `${window.location.origin}/oauth2/authorize?client_id=${args[0]["user"]["id"]}&scope=bot&disable_guild_select=true&guild_id=${guild.id}`,
                  "title",
                  "DISCORD_BOT_POPOUT"
                );
                (
                  await waitForElement(
                    "div.footer-3ZalXG > button.button-38aScr.lookLink-9FtZy-",
                    win
                  )
          ...
          res.props.children.props.children.push(
            React.createElement(
              ...
              React.createElement(menu.MenuItem, {
                ...
                action: async () => {
                  const win = await popout(
                    `${window.location.origin}/oauth2/authorize?client_id=${args[0]["user"]["id"]}&scope=bot`,
                    "title",
                    "DISCORD_BOT_POPOUT"
                  );
                  (
              ...
              React.createElement(menu.MenuItem, {
                ...
                action: async () => {
                  const win = await popout(
                    `${window.location.origin}/oauth2/authorize?client_id=${
                      args[0]["user"]["id"]
                    }&scope=bot&permissions=${await this.getPermissionsRaw(
                      args[0]["guildId"],
                      args[0]["user"]["id"]
                    )}`,
                    "title",
